 ...
 *
 * Defines the public logging API for the Utilities library, including log levels,
 * log sink callback, and functions to set/get the log sink and log level. The
 * logging system is designed to be flexible and efficient, allowing users to
 * customize how log messages are handled and filtered based on their severity. The
 * log sink callback can be used to route log messages to different outputs, such as
 * console, file, or platform-specific logging systems. The log level can be set at
 * runtime to control which messages are emitted, with the option to completely disable
 * certain levels for performance reasons.
 *
 * @bug	    No known bugs.
 */
 /*****************************************************************************/
